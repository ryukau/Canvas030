class WaveView extends Canvas {
  ...
  drawWaveWide() {
    ...
    var interval = Math.floor(this.length / this.width)
    var limit = this.offset
    for (var x = 0; x < this.width; ++x) {
      var index = limit
      limit += interval
      limit = Math.min(limit, this.wave.length)

      var cur = new Vec2(x, this.fixY(this.wave[index - 1]))
      var next = new Vec2(x, this.fixY(this.wave[index]))
      this.drawLine(cur, next)
